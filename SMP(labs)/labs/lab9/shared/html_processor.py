...
class HtmlProcessor:
    ...
    def __init__(self, folder_path: str):
        ...
        Returns:
            None
        """
        self._files_data = self._read_files_in_folder(folder_path)
        logger.info("Read %s HTML files.", len(self._files_data))
    ...
    @staticmethod
    def _read_files_in_folder(folder_path: str):
        """
        Reads the contents of all HTML files in the specified folder.
    ...
    def get_text_data(self, page_number: int):
        ...
        """
        if 1 <= page_number <= len(self._files_data):
            _, content = self._files_data[page_number - 1]
